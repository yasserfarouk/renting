...
class Tjaronchery10Agent(DefaultParty):
        ...
        if self.minicount == 0:
            try:
                ...
                try:
                    ...
                    self.logger.log(logging.INFO, "file does not exist1 :(")
            except FileNotFoundError:
                self.logger.log(logging.INFO, "file does not exist counter :(")
        self.minicount = 1
    ...
    def save_data(self):
        ...
        with open(f"{self.storage_dir}/{y}datatactic.txt", "a") as t:
            t.close()
        if self.counter == 0:
            self.logger.log(logging.INFO, "OPEN FILEEEEEEEEE")
